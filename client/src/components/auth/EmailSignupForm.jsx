...
const EmailSignupForm = () => {
  const { openAuthModal } = useAuth(); // <-- Get the function from context
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      openAuthModal('login'); // Switch to login view on success
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="mt-8">
        <a href={`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000'}/api/auth/google`} className="w-full flex justify-center items-center gap-3 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
          <GoogleIcon />
      ...
      <form onSubmit={handleSignup} className="space-y-4">
        <div>
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Email</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
        ...
        <div>
          ...
          <select value={country} onChange={(e) => setCountry(e.target.value)} required className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          <button type="submit" className="w-full py-2.5 mt-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 font-semibold" disabled={loading}>
      ...
      <p className="mt-6 text-sm text-center text-gray-600">
        ...
        <button onClick={() => openAuthModal('login')} className="font-medium text-blue-600 hover:underline focus:outline-none">
          Sign In
        </button>
      </p>
    </div>
